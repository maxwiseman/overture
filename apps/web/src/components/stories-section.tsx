...
export function StoriesSection({ edition, query }: { edition: PublicationEdition; query: string }) {
  ...
  return (
    <section
      className="mx-auto max-w-[1440px] px-[42px] pt-[70px] pb-[90px] text-foreground max-[800px]:px-5 max-[800px]:pt-[54px] max-[800px]:pb-[70px]"
      id="next-stories"
    >
      <div className="flex items-end justify-between gap-8 border-publication-border border-b pb-7 max-[800px]:flex-col max-[800px]:items-start">
        <div>
        ...
        <p className="mb-2 max-w-[350px] leading-[1.6] text-publication-muted max-[800px]:m-0">
          {query
      ...
      <div className="grid grid-cols-2 max-[800px]:grid-cols-1">
        {storiesForGrid.map((story, gridIndex) => {
          ...
          return (
            <article
              className={`group grid min-h-[390px] grid-cols-[minmax(180px,.85fr)_minmax(220px,1fr)] gap-8 border-publication-border border-b py-[38px] pr-[38px] max-[800px]:grid-cols-1 max-[800px]:gap-6 max-[800px]:px-0 max-[800px]:py-[30px] ${gridIndex > 0 ? "border-l pl-[38px] max-[800px]:border-l-0 max-[800px]:pl-0" : ""}`}
              key={story.id}
            >
              <Link
                className="relative min-h-[310px] overflow-hidden bg-publication-placeholder max-[800px]:min-h-[300px]"
              ...
              <div className="flex flex-col">
                <div className="flex gap-[18px] text-[0.69rem] tracking-[0.13em] text-publication-accent uppercase">
                ...
                <p className="m-0 leading-[1.55] text-publication-muted max-[800px]:mb-7">{story.deck}</p>
                <div className="mt-auto flex items-center justify-between text-[0.82rem] text-publication-muted [&_button]:cursor-pointer [&_button]:border-0 [&_button]:bg-transparent [&_button]:p-[7px]">
        ...
        {storiesForGrid.length === 0 ? (
          <p className="col-span-full py-14 text-publication-muted">No stories found in this issue.</p>
        ) : null}
      </div>
      <Link
        className="mt-9 ml-auto flex w-max items-center gap-[9px] text-publication-accent max-[800px]:ml-0"
